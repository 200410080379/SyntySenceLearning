...
def audit(layout, geometry):
    size, grid_size = float(layout['cell_size_cm']), layout['grid_size']
    if type(grid_size) is int:
        nx = ny = grid_size
    elif isinstance(grid_size, (list, tuple)) and len(grid_size) == 2:
        nx, ny = grid_size
    else:
        raise ValueError('grid_size must be a positive integer or [columns, rows]')
    if any(type(count) is not int or count <= 0 for count in (nx, ny)):
    ...
    for tile in layout['tiles']:
        i, j = tile['i'], tile['j']
        if type(i) is not int or type(j) is not int or not (0 <= i < nx and 0 <= j < ny):
            errors.append({'reason': 'tile outside declared integer grid', 'tile': tile_brief(tile)})
            continue
        cells[i, j].append(tile)
    missing = [[i, j] for i in range(nx) for j in range(ny) if (i, j) not in cells]
    ...
    expected_seams = nx * (ny - 1) + ny * (nx - 1)
    failures = [seam for seam in seams if not seam['passed']]
    coverage_ok = not (missing or duplicates or errors) and len(cells) == nx * ny
    ...
    return {'passed': passed, 'units': 'centimeters',
            ...
            'coverage': {'grid_size': grid_size, 'expected_tile_count': nx * ny,
...
def self_test():
    ...
    # Six explicit cells form a 2-column, 3-row rectangle with seven shared
    # borders. Their nonzero origin also catches accidental world/grid mixing.
    rectangle = {
        'cell_size_cm': 500, 'grid_size': [2, 3], 'origin_cm': [-1000, 2000, 0],
        'tiles': [
            {'i': 0, 'j': 0, 'mesh': 'plane', 'yaw': 0, 'location': [-500, 2500, 0]},
            {'i': 1, 'j': 0, 'mesh': 'plane', 'yaw': 0, 'location': [0, 2500, 0]},
            {'i': 0, 'j': 1, 'mesh': 'plane', 'yaw': 0, 'location': [-500, 3000, 0]},
            {'i': 1, 'j': 1, 'mesh': 'plane', 'yaw': 0, 'location': [0, 3000, 0]},
            {'i': 0, 'j': 2, 'mesh': 'plane', 'yaw': 0, 'location': [-500, 3500, 0]},
            {'i': 1, 'j': 2, 'mesh': 'plane', 'yaw': 0, 'location': [0, 3500, 0]},
        ],
    }
    rectangular = audit(rectangle, {'plane': plane})
    assert rectangular['passed']
    assert rectangular['coverage']['expected_tile_count'] == 6
    assert rectangular['summary']['expected_adjacent_seams'] == 7
    assert rectangular['summary']['checked_adjacent_seams'] == 7
    assert Counter(seam['axis'] for seam in rectangular['seams']) == {'X': 3, 'Y': 4}
    missing = audit({**rectangle, 'tiles': rectangle['tiles'][:-1]}, {'plane': plane})
    assert not missing['passed'] and missing['coverage']['missing_cells'] == [[1, 2]]
    assert missing['summary']['checked_adjacent_seams'] == 5
    # Keep six inputs while replacing the top-right cell with a duplicate:
    # checking only input count or bounding extents must not accept this layout.
    duplicated = audit({**rectangle, 'tiles': rectangle['tiles'][:-1] + [rectangle['tiles'][0]]},
                       {'plane': plane})
    assert not duplicated['passed']
    assert duplicated['coverage']['input_tile_count'] == 6
    assert duplicated['coverage']['missing_cells'] == [[1, 2]]
    assert [entry['cell'] for entry in duplicated['coverage']['duplicate_cells']] == [[0, 0]]
    out_of_bounds = audit({**rectangle, 'tiles': rectangle['tiles'] + [
        {'i': 2, 'j': 0, 'mesh': 'plane', 'yaw': 0, 'location': [500, 2500, 0]}]}, {'plane': plane})
    assert not out_of_bounds['passed'] and len(out_of_bounds['coverage']['errors']) == 1
    square = audit({**rectangle, 'grid_size': 2, 'tiles': rectangle['tiles'][:4]}, {'plane': plane})
    assert square['passed'] and square['summary']['checked_adjacent_seams'] == 4
    print('SELF_TEST_PASS: sculpted profile, step, hole, horizontal drift, envelope crossing, '
          'four rotations, rectangular occupancy, missing/duplicate/outside cells, square compatibility')
