...
**本次确认修复了这组 Landscape 纹理缺口及棋盘格问题，尚未证明完整原图已还原。** 209 个空网格组件仍存在；粉色餐厅等颜色是否符合原效果也未确认。自建小城的通过结果只说明本次重开后的实例与引用和配方一致，不是对更新模型的几何、碰撞、导航或性能重新验收。旧截图、旧依赖清单和历史验证记录保留，用于区分各次环境状态。
...
这张 Woodland 历史图拍摄于 Alpine 1.1.0 更新前，可见屋顶地被模型，也存在粉色建筑、白色植被和棋盘格地表等依赖缺失表现。该图只用于结构定位，不能作为升级后状态或原包正常材质、配色与最终氛围的还原证据。
